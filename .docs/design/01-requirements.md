...
**제약**
- 주문 생성·재고 차감은 단일 트랜잭션. 어느 한쪽 실패 시 전체 롤백(All-or-Nothing). 실패 이력 보존·자가 회복은 다음 라운드 동시성/실패복구 주제.
- 스냅샷 필드 = `productId`, `productName`, `unitPrice`, `quantity` 4개로 한정.
- 본 라운드 흐름: **주문 생성(`CREATED`) → 재고 차감 → 단일 트랜잭션 커밋**. 실패 시 예외 전파 + 전체 롤백. `SUCCEEDED`/`FAILED` 상태와 결제 합류는 다음 라운드에서 다룬다.
...
| D12 | **물리적 FK 제약(DB constraint)은 사용 안 함**. 같은 애그리거트 내부에서만 JPA `@ManyToOne`/`@OneToMany` 매핑을 유지(D18). 다른 애그리거트 사이는 ID 참조 | INSERT/UPDATE 성능, 데드락 회피, CASCADE 위험 제거. 무결성은 애플리케이션 레이어에서 검증 |
...
| D18 | **JPA 연관관계 매핑 규칙 (멘토 룰)** — *같은 애그리거트* 안은 `@OneToMany`/`@ManyToOne`로 *물리적 매핑*하고, *다른 애그리거트* 사이는 ID 참조(*논리적 매핑*)만 한다. 적용: Order↔OrderItem (같은 애그리거트 → 매핑 O), OrderItem↔Product / Product↔Brand / Stock↔Product / Like↔Product (다른 애그리거트 → ID 참조). N+1은 JPA의 문제로 매핑 자체를 피할 이유가 아니다 — 같은 애그리거트는 같이 살고 같이 죽으므로 *그래프 탐색의 편의*를 도메인이 누리는 게 자연스럽다 | 같은 애그리거트(라이프사이클이 묶인 단위)는 cascade·orphanRemoval의 부작용이 *의도된 작동*이라 관리 비용이 낮음. 반대로 다른 애그리거트 사이에 매핑을 두면 *cascade/Fetch 전략/detached 처리*가 도메인 경계를 가로질러 새어 들어와 관리 비용이 폭증한다 |
