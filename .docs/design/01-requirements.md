...
| D18 | **주문 고아 row 보정은 다음 라운드(결제 합류)에서 해결** — `OrderFacade.placeOrder`는 `placeInitial` / `decreaseAll` / `markSucceeded`가 각각 별도 트랜잭션이라, `decreaseAll` 또는 `markSucceeded` 자체가 죽으면 *재고는 빠졌는데 status=CREATED로 남는* 좀비 row 윈도우가 가능. 본 라운드는 `catch (RuntimeException)`까지 넓혀 *가능한 한 모든 실패*를 FAILED 이력으로 흡수하고, 자가 회복(스케줄러 보정 / Outbox + AFTER_COMMIT / Saga)은 결제 라운드 합류 시점에 합쳐 설계한다 | 본 라운드 스코프 제한 + 결제가 합류하면 *어차피 보상 트랜잭션 / 외부 I/O 분리*가 필요해 그때 함께 설계하는 게 변경 비용이 적다. `markFailed` 자체가 실패할 가능성은 본 라운드 범위 밖 — 결제 라운드의 회복 메커니즘에 흡수된다 |
