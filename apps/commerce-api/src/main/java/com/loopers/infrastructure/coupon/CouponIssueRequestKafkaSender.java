...
public class CouponIssueRequestKafkaSender implements CouponIssueRequestSender {
    ...
    public void send(String requestId, Long userId, Long couponTemplateId) {
        try {
            ...
            log.warn("쿠폰 발급 요청 발행 중단 (templateId={}): {}", couponTemplateId, e.getMessage());
            throw new CoreException(ErrorType.INTERNAL_ERROR, "쿠폰 발급 요청 접수에 실패했습니다. 잠시 후 다시 시도해주세요.", e);
        } catch (Exception e) {
            log.error("쿠폰 발급 요청 발행 실패 (templateId={}, {}): {}", couponTemplateId, e.getClass().getSimpleName(), e.getMessage());
            throw new CoreException(ErrorType.INTERNAL_ERROR, "쿠폰 발급 요청 접수에 실패했습니다. 잠시 후 다시 시도해주세요.", e);
        }
    }
}
