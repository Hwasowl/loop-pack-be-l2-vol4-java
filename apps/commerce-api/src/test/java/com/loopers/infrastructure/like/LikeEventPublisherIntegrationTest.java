...
class LikeEventPublisherIntegrationTest {
    ...
    void setUp() {
        ...
        productId = productRepository.save(new ProductModel(brand.getId(), "후드", "포근함", 49_000L)).getId();
    }
    ...
    void publishesSnapshotAndPlusOneDelta_onLike() {
        ...
        // then
        CatalogEventPayload liked = awaitLikeEvents(1).get(0);
        assertThat(liked.productId()).isEqualTo(productId);
    ...
    void publishesSnapshotAndMinusOneDelta_onUnlike() {
        ...
        List<CatalogEventPayload> events = awaitLikeEvents(2);
    ...
    void publishesNothing_whenLikeIsIdempotent() {
        ...
        verify(kafkaTemplate, after(500).times(1))
            .send(eq(KafkaTopics.CATALOG_EVENTS), any(), argThat(this::isLikeEvent));
    }

    /**
     * 발행이 @Async라 기대 건수가 도착할 때까지 기다린 뒤 페이로드를 모은다.
     * catalog-events에는 다른 publisher(조회 등)의 이벤트도 섞이므로 좋아요 타입만 세고 걸러낸다.
     */
    private List<CatalogEventPayload> awaitLikeEvents(int expected) {
        ...
        List<CatalogEventPayload> events = captor.getAllValues().stream()
            ...
            .filter(p -> "PRODUCT_LIKE_COUNT_CHANGED".equals(p.eventType()))
            .toList();
        assertThat(events).hasSize(expected);
        return events;
    }

    private boolean isLikeEvent(Object value) {
        return value instanceof CatalogEventPayload p && "PRODUCT_LIKE_COUNT_CHANGED".equals(p.eventType());
    }
}
